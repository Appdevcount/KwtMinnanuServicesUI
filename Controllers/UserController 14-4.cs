    ...
    public class UserController : MyBaseController
    {
...

        {
            try
            {
                ...
                if (Id == null && Requesttype == null && purpose == null && reqnum == null && !CanCreateOrg)
                {
                    ...
                    return RedirectToAction("UnAuthorizedAction", "Registration",new { MsgCode=1 });
                }
        ...
        public ActionResult Org_Registration(OrgGetBasicResult objorgreg)
        {
            try
            {
                ...
                if ((objorgreg.OrganizationRequestId == null || objorgreg.OrganizationRequestId == 0 ) && Session["purpose"].ToString() !="0")//Check to ensure  that the entered trade license is not used already for any organization(existing,new,orgrequests) -Siraj
                {
                    ReqObj ro = new ReqObj { CommonData = objorgreg.TradeLicNumber };
                    ...
                    if (DT != null)
                    {
                        if (DT.Rows.Count > 0)
                        {
                            if (!Convert.ToBoolean(DT.Rows[0]["UniqueLicense"]))
                            {
                                ...
                                //ModelState.AddModelError(string.Empty, Resources.Resource.AlreadyRegisteredOrgLicense);//+ e.Message);
                                ModelState.AddModelError("TradeLicNumber", Resources.Resource.AlreadyRegisteredOrgLicense);//+ e.Message);
                                //ViewBag.modelerror = Constantclass.number;
            ...
            try
            {
                ...
                if (Session["purpose"].ToString().Trim() == Constantclass.number1)
                {
                    ...
                    else
                    {
                        ...
                        objOrgRequestdatalist[0].Data.OrgGetImportLicenseResult[0].Year = objimporterdetails.Year ;//added Siraj - temporary license details(lic and year coming as empty when navigate to next and come back)
                    }
        ...
        public ActionResult UploadDocument(OrgGetDocumentsResult obj)
        {
            ...
            catch (Exception e) { return Json("Error While Saving."); }
            checksession();
            return View();
        }
            ...
            try
            {
                ...
                if (TempData["MyAccount"] != null)
                {
                    ...
                    if (dtMyAccountlist.Rows.Count > 0)
                    {
                        ...
                        if (String.IsNullOrEmpty(ObjMyaccountinput.LicenseNumber) || String.IsNullOrEmpty(ObjMyaccountinput.CivilId))
                        {
