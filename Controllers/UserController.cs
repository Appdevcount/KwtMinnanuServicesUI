...
namespace WebApplication1.Controllers
{
    public class UserController : MyBaseController
    {
...

        {
            ...
            try
            {
                ...
                if (purpose != null)
                {
                    ...
                    Session["mypurpose"] = mypurpose;
                }
                else
                {
                    mypurpose = string.Empty;
                    Session["mypurpose"] = "test";
                }
                ...
                {
                    Session["OrgRegistrationAllowed"] = true;
                    if (NewRequest)//added Siraj - clearing the tempdata and OrgReqId which was assigned in last request
                    {
                        //Message handling
                        //if (Session["LegalEntity"].ToString() == "2")
                        {
                            ReqObj rq = new ReqObj { CommonData = Session["LicenseNumberOfLoggedInUser"].ToString() };
                            DataTable ExistingOrgRequestFortheUser = objdataclass.ExistingOrgRequestFortheUser(rq);
                            string UserRequestType = ExistingOrgRequestFortheUser.Rows[0]["RequestType"].ToString();
                            if (UserRequestType == "OrgAlreadyCreated")
                            {
                                return RedirectToAction("UnAuthorizedAction", "Registration", new { MsgCode = 5 });
                            }
                            else if (UserRequestType == "HasEtradeRequest")
                            {
                                return RedirectToAction("UnAuthorizedAction", "Registration", new { MsgCode = 4 });
                            }
                            else if (UserRequestType == "HasEservicesRequest")
                            {
                                return RedirectToAction("UnAuthorizedAction", "Registration", new { MsgCode = 3 });
                            }
                            else if (UserRequestType == "NEW")
                            {
								//proceed further
                                //return RedirectToAction("UnAuthorizedAction", "Registration");
                            }
                    ...
                    ObjOrgGetBasicResult.OrgAraName = Session["CompanyName"].ToString();
                }
        ...
        public ActionResult Org_Industrial(OrgGetIndustrialResult objindustrialdetails)
        {
            try
            {
                ...
                if (Session["purpose"].ToString().Trim() == Constantclass.number1)
                {
                    ...
                    /*commented by azhar on 11\03\2019 on reqst by mohd for relaxing validtation in checking years
        ...
        public ActionResult Org_Commercial(OrgGetCommercialLicenseResult objcommercialdetails)
        {
          ...
          [NonAction]
        public bool uploadmethod()
        {//Modified return type and logic as part of stabilization and enhancements  -Siraj
            try
            {
                if (Session["purpose"].ToString().Trim() == Constantclass.number1)
                {
                ...
                if (objOrgRequestdatalist[0].Data.OrgGetBasicResult[0].RequestType == "0")//added Siraj .. Name change doc is not required to be uploaded for first org creation
                {
                    string NameChangeTypeid = ConfigurationManager.AppSettings["docTypeOrganizationNameChangeDocument"].ToString();
                    //DocTypeslist docTypeslist = objDocTypes.FirstOrDefault();
                    //List<DocTypes> dtypes = docTypeslist.DocTypes;
                    objDocTypes.FirstOrDefault().DocTypes.RemoveAll(i => i.TypeId == NameChangeTypeid);
            ...
            catch (Exception ex)
            {
                ...
                //throw ex;
                CommonFunctions.LogUserActivity("uploadmethod", "", "", "", "", ex.Message.ToString());
        ...
        public ActionResult UploadDocument()
        {
            ...
            try
            {
                ...
                bool DocTypes_Docs_MissedDocs_Retrieved =  uploadmethod();

                if (!DocTypes_Docs_MissedDocs_Retrieved)
                {
        ...
        //Delete method below obsole now - siraj
        ...
        public JsonResult DeleteFileFromServer(string OrgReqDocId)
        {//Added new method for file deletion instaed of using UploadDocument1 - Siraj .. This is called in Ajax
            try
            {
                ...
                if (Session["UserId"] == null)
                {
                    var res =new  { Status="-1",Description="UnAuthorized" };
                ...
                var res1 = new { Status = "1", Description = "Success" };
            ...
            catch (Exception e)
            {
                ...
                var res2 = new { Status = "-2", Description = e.Message };
                return Json(res2, JsonRequestBehavior.AllowGet);
            }
        }

        [HttpPost]
        public ActionResult UploadDocument(OrgGetDocumentsResult obj)//,string dummyargnotusedjusttodifferntiateargscount)
        {//Modified method and logic as part of stabilization and enhancements  -Siraj
            bool DocTypes_Docs_MissedDocs_Retrieved;
            try
            {
                if (Session["UserId"] == null)
                {
                ...
                if (Session["DocTypeslist"] != null)
                {
                    ...
                    {
                        //Custom File validations now configurable in Code - Siraj
                        HttpCookie langCookie = Request.Cookies["culture"];

                        int DocumentPosition;

                        if (objOrgRequestdatalist[0].Data.OrgGetCommercialLicenseResult[0].CommLicType.ToLower().Contains("personal"))
                        { 
                            if(langCookie.Value.Contains("ar"))
                                DocumentPosition = objDocTypeserror.IndexOf("عقد تأسيس") ;
                            else
                                DocumentPosition = objDocTypeserror.IndexOf("License Certificate"); 

                            if(DocumentPosition >= 0)
                                objDocTypeserror.RemoveAt(DocumentPosition);
                        }
                       else if (objOrgRequestdatalist[0].Data.OrgGetCommercialLicenseResult[0].CommLicType.ToLower().Contains("corporation"))
                        { 
                            if (langCookie.Value.Contains("ar"))
                                DocumentPosition = objDocTypeserror.IndexOf("ترخيص وزارة التجارة و الصناعة");
                            else
                                DocumentPosition = objDocTypeserror.IndexOf("MOCI Certificate");

                            if (DocumentPosition >= 0)
                                objDocTypeserror.RemoveAt(DocumentPosition);
                        }
                        if (objOrgRequestdatalist[0].Data.OrgGetBasicResult[0].RequestType == "0")//Name change doc is optional during Update request .. and it should not be visible  during first org creation
                        {
                            if (langCookie.Value.Contains("ar"))
                                DocumentPosition = objDocTypeserror.IndexOf("تغيير اسم المنظمة كتاب");
                            else
                                DocumentPosition = objDocTypeserror.IndexOf("Organization Name Change Document");

                            if (DocumentPosition >= 0)
                                objDocTypeserror.RemoveAt(DocumentPosition);
                        }
                        //Still if there is any thing missed
                        if (objDocTypeserror.Count > 0)
                        {
            ...
            catch (Exception e)
            {
                WriteToLogFile(e.Message + e.StackTrace, "UploadDocument_submit");
                 DocTypes_Docs_MissedDocs_Retrieved = uploadmethod();
          ...
          public ActionResult RenderDocument(string DocumentId)
        {//Added new method for document rendering in view - Siraj
            if (Session["UserId"] == null)
            {
                return RedirectToAction("Error", "error");
            }
            string referenceprofile = "OrganizationRequests";
            ...
            if (!DocumentId.All(char.IsDigit))
            {
                if (DocumentId.Contains('%'))
                {
                    id = CommonFunctions.CsUploadDecrypt(System.Web.HttpUtility.UrlDecode(DocumentId));
                }
                else
                {
                    id = CommonFunctions.CsUploadDecrypt(DocumentId);
                }
            ...
            if (id != "")
            {
                try
                {
                    ...
                    objDownloadFile.tokenId = Session["UserId"].ToString();
                ...
                catch (Exception ex)
                {
                    ...
                    CommonFunctions.LogUserActivity("Error viewing OrgDocId=" + id, "", "", "", "", ex.Message.ToString());
                }

            }
            ViewBag.Errormessage = "Error In Viewing the File";
            return RedirectToAction("Error", "Error");
        ...
        public ContentResult Requestprint(string RequestNumber)
        {
            ...
            try
            {
                ...
                // added by azhar for security issue
                Bu.mobileUserId = int.Parse(Session["UserId"].ToString());
        ...
        public ActionResult MyAccount()
        {
            try
            {
                ...
                ds = objdataclass.Myaccount(objSecurityParams);

                dtMyAccountlist = ds.Tables[0];
                if (ds.Tables.Contains("CompanyCivilIddetails"))
                {
                    if (ds != null && ds.Tables["CompanyCivilIddetails"].Rows.Count > 0)

                    {
                        var DropdownBind = from s in ds.Tables["CompanyCivilIddetails"].AsEnumerable()
                                           select new SelectListItem
                                           {
                                               //     Text = s["Name"].ToString() + " (" + s["Cnt"] + ") ",
                                               Text = s["name"].ToString(),
                                               // Value = CommonFunctions.CsUploadEncrypt(s["tradelicensenumber"].ToString())
                                               Value = s["tradelicensenumber"].ToString(),

                                               //  Selected = (s["DeclarationDocumentId"].ToString() == DocumentId ? true : false)

                                           };
                        //    ObjbrokerRenwalModel.ddlDocumentTypesitems = DropdownBind.ToList();
                        ViewBag.CompanyCivilIddetails = DropdownBind.ToList();
                    }
                    else
                    {
                    }
                }
                //     Session["CompanyCivilIddetails"] = ds.Tables["CompanyCivilIddetails"];


                //   ViewBag.CompanyCivilIddetails= ds.Tables["CompanyCivilIddetails"];
