...
$().ready(function () {
    ...
    //$('#NewBrokerUserMsg').css('display', 'none');
...
$("#ExistingUser").change(function () {
    if (this.checked) {
        ...
        $('#NewBrokerUserMsg').css('display', 'block');
...
function RegSuccess(data) {
 ...
 var successMsg = "تم تسجيل المستخدم بنجاح، بعد قليل تصلك رسائل بأرقام التحقق لتفعيل رقم الجوال وحساب البريد الالكتروني";

        var language = getLang("culture");

   if (language.indexOf("en") != -1) {
            successHeader = "Message";
            successMsg = "Sign up done successfully, shortly you will receive OTP codes to verify your email and mobile number";
        }
...
function RegFailure(jqXHR, exception) {
    var msg = '';
    if (jqXHR.status === 0) {
        msg = 'Not connect.\n Verify Network.';
...
$('input[name=ImporterLicenseNumber]').on('mouseup', function (e) {

    //  if (code == 9 || code == 13)
    if (StoredCivilid != $("#CompanyCivilId").val()) {
        civilidverifeidformobile = false;
    }

    if (civilidverifeidformobile != true && $("#CompanyCivilId").val() != '') {

        getmocidetails('civilid');

    }
    StoredCivilid = $("#CompanyCivilId").val();

});

$('input[name=Block]').on('mouseup', function (e) {
    var code = e.keyCode || e.which;
    //  if (code == 9 || code == 13)
    {
        getmocidetails('ImporterLicenseNumber');
...
if (isMobile.any()) {
    var tabpressed;
    $('input[name=CompanyCivilId]').on('blur', function (e) {
        var code = $("#CompanyCivilId").val();
        if (code !="")
        {
            if (civilidverifeidformobile != true)
                {
            getmocidetails('civilid');
            }
        }
    ...
    $('input[name=ImporterLicenseNumber]').on('blur', function (e) {
...
function getmocidetails(reftype) {
    ...
    if (reftype == 'civilid') {
        ...
        StoredCivilid = $('#CompanyCivilId').val();
        if ($('#CompanyCivilId').prop('readonly')) {
     ...
     //   document.addEventListener('keyup', function (event) {
          //  if (event.keyCode == 9 )
            ...
            if ($("#TradeLicenseNumber").val() == "" )
               ...
               $.ajax({
                    type: "post",
                   url: applicationUrl+"registration/GetCivilIDDetailsFromMoci",
                    ...
                    success: function (data) {
                        ...
                        if (data[0] != null) {
                            if (data[0].ImporterLicenseNumber.length != '0') {
                             ...
                             //if (data[0].CompanyName.length != '0') {
                            ...
                            if ((data[0].TradeLicenseNumber.val == 'undefined' || data[0].TradeLicenseNumber.length == '0')) {
                                ...
                                var isnum = /^\d+$/.test($('#CompanyCivilId').val());

                                if ($('#CompanyCivilId').val() != '' && isnum == true) {
                                    $('#CompanyCivilId').prop('readonly', true);
                                }
...
$(document).ready(function () {
    $('#Governorate').change(function () {
        $.ajax({
            type: "post",
            url: applicationUrl+"registration/GetCitiesofGovernorate",
...
function trigger() {
    ...
    $("#ExistingUser").trigger('click');
}

$('#EmailId').focus(function () {
    debugger;
    document.getElementById("EmailRefmsg").style.display = "block";
});

$('#EmailId').focusout(function () {

    document.getElementById("EmailRefmsg").style.display = "none";

});

$('#MobileTelNumber').focus(function () {

    document.getElementById("MobileNumberRefmsg").style.display = "block";
});

$('#MobileTelNumber').focusout(function () {



    document.getElementById("MobileNumberRefmsg").style.display = "none";
});


//var legalentity = '@ViewBag.SelectedEntity';
if (legalentity == '1') {
    $(window).bind("load", function () {

        $('#ExistingUser').prop('checked', true);
        if ($('#ExistingUser').is(':checked')) {
            $(".ClearingAgentPOPT").removeClass('ClearingAgentNP').addClass('ClearingAgentP');
            $(".ClearingAgentExistingUser").removeClass('ClearingAgentP').addClass('ClearingAgentNP');
            $('.ClearingAgent').attr('data-val', false);


            //$('#NewBrokerUserMsg').attr('style', 'display:none');
            $('#NewBrokerUserMsg').css('display', 'none');
        }
